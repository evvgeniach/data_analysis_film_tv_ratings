...
mean_scores_by_genre <- movies_shows %>%
  ...
  group_by(genres) %>%
  summarise(
    mean_imdb_by_genre = mean(imdb_score, na.rm = TRUE),
...
movies_shows <- movies_shows %>%
  rowwise() %>%
  mutate(
    imdb_score = ifelse(is.na(imdb_score) & length(unlist(genres)) > 0, mean(mean_scores_by_genre$mean_imdb_by_genre[mean_scores_by_genre$genres %in% unlist(genres)], na.rm = TRUE), imdb_score),
...
movies_shows <- movies_shows %>%
  rowwise() %>%
  mutate(
    imdb_score = ifelse(length(unlist(genres)) == 0, mean_imdb_score, imdb_score),
...
joined_data <- joined_data %>%
  mutate(
    weighted_imdb = imdb_score * imdb_votes,
...
director_ratings <- joined_data %>%
  ...
  mutate(
    avg_weighted_imdb = sum_weighted_imdb / total_imdb_votes,
...
q2<- q2[1:10,]
q2<- q2 %>%
  mutate(popularity = (imdb_votes + tmdb_popularity)/2)
...
set.seed(3222)
trainIndex <- createDataPartition(dataX$imdb_score, p = .8, list = FALSE)
...
# Predict IMDb score for a movie with runtime longer than 120 minutes
pred_long_runtime <- predict(mod, newdata = data.frame(runtime_minutes = 140))

# Predict IMDb score for a movie with runtime shorter than 120 minutes
pred_short_runtime <- predict(mod, newdata = data.frame(runtime_minutes = 110))
